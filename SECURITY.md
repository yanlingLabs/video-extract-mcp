...
- **It downloads and extracts speech models on demand**, the first time a video needs local transcription: `.tar.bz2` archives from the sherpa-onnx GitHub release, extracted with `tar` into the models directory. The source URL is overridable by environment variable only, never by a caller, and the archive is verified to contain the files the recognizer opens before it is put in place — a truncated or wrong archive is discarded rather than published under its final name. `VIDEO_EXTRACT_AUTO_FETCH_MODELS=0` disables it entirely.
