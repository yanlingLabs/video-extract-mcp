...
- **The notification is unverified.** `presentSignIn` asks `osascript` for one, and the test proves only that it was asked. macOS attributes it to Script Editor and can suppress it silently.
- **The sign-in page's final design was seen in Safari only as a demo copy** (a page with no wait behind it). The earlier card design ran end to end in Safari for a real WeChat sign-in. Headless Chrome screenshots covered light mode and several widths; dark mode was only seen in the earlier design. The sign-in URLs for the nine known sites all answered 200 when checked (2026-09-23); sites move them, and a stale one only degrades to a page that still has a sign-in link.
